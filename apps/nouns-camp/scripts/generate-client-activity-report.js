...
function processWithJq(data, filter) {
  ...
  try {
    const result = execSync(`cat ${tempFile} | jq '${filter}'`, {
      encoding: "utf8",
    }).trim();
    return result;
  } finally {
...
function fetchVotingData(startTimestamp, endTimestamp) {
  ...
  const totalVotes = parseInt(
    processWithJq(totalVotesResult, ".votes | length"),
  );
  ...
  const powerVotes = parseInt(
    processWithJq(powerVotesResult, ".votes | length"),
  );
  ...
  const totalVotingPower = clientData.reduce(
    (sum, client) => sum + client.totalVotingPower,
    0,
  );
  ...
  for (const client of clientData) {
    ...
    const voterIds = JSON.parse(
      processWithJq(votersResult, ".votes | map(.voter.id)"),
    );
  ...
  let proposalsByClient = JSON.parse(
    processWithJq(proposalsResult, proposalDataJq),
  );
...
function generateFinalSummary(data) {
  ...
  if (proposalsByClient.length > 0) {
    ...
    const proposerPercent = (
      (mostActiveProposerClient.count / totalProposals) *
      100
    ).toFixed(1);
    const proposerName =
...
async function generateReport() {
  try {
    ...
    const summarySection = generateSummaryReport(
      data,
      startTimestamp,
      endTimestamp,
    );
    ...
    const fullReport =
      summarySection + tableSection + proposalsSection + finalSection;
